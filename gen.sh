...
function instance_fastd {
	...
	replace /etc/fastd/$3 nef27grd $3
...
function instance_wgvxlan {
	...
	export nef27grd=$3
	EXPVARS='$PRIVATE_KEY:$SERVER_PUBLIC_KEY:$SERVER_ENDPOINT:$ALLOWED_IPS:$IPV6_ADDR:$VXLAN_ID:$nef27grd'
	...
	ip a s bat-$nef27grd >/dev/null 2>&1
	if [ $? -eq 0 ]; then
		ip link del bat-$nef27grd
	fi
	ip a s vxlan-$nef27grd >/dev/null 2>&1
	if [ $? -eq 0 ]; then
		ip link del vxlan-$nef27grd
	fi
	ip a s wg-$nef27grd >/dev/null 2>&1
	if [ $? -eq 0 ]; then
		ip link del wg-$nef27grd
	fi

	# connect to wireguard (vxlan and bat interfaces are handled by wg-quick)
	systemctl restart wg-quick@wg-$nef27grd
	systemctl enable wg-quick@wg-$nef27grd
}

function instance_yanic {
	cp templates/instance/yanic.conf /etc/yanic/$3.conf
	replace /etc/yanic/$3.conf nef27grd $3
	replace /etc/yanic/$3.conf 8141 $5
...
function instance_nginx {
	...
	replace $WEBCONF/$4.conf 8141 $5
	replace $WEBCONF/$4.conf nef27grd $3
}
...
function group_webdir {
	...
	for nef27grd in ${3//,/ }
	do
		URL=$(cat $HOME/sites | awk "\$3 == \"$nef27grd\"" | cut -d' ' -f4)
		if [ "$DATASOURCES" == "" ]
		then
	...
	replace $WEBDATA/$4 nef27grd ${3//,/\\&var-job=}
...
function alias_nginx {
	...
	replace $WEBCONF/$2.conf 8141 $(echo $LINE | cut -d' ' -f5)
